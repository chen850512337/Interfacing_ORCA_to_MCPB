"""
There are four methods in this module to get the force constants of the bond,
...
3) Modified seminario method, which is a method developed by Dan Cole and
co-workers which could give better results about the angle force constants
than the Seminario method

4) Z-matrix method, which uses the whole Hessian Matrix to generate the force
constants.
"""
...
# chenjf>
# add orca
from pymsmt.mol.orcaio import get_crds_from_orca, get_matrix_from_orca
...
from numpy import average, array, dot, cross, std, zeros
...
#-----------------------------------------------------------------------------
# Empirical method for zinc, Ref: Yu, Li and Merz. JCTC, 2018, 14, 242-254
#-----------------------------------------------------------------------------
...
def gene_by_empirical_way(smpdbf, ionids, stfpf, pref, finf):
    ...
    print("==================================================================")
    print("To cite the empirical method please use the following reference:")
    print("  Yu, Li, and Merz, J. Chem. Theory Comput., 2018, 14, 242-254.")
...
def gene_by_QM_fitting_sem(smpdbf, ionids, xstru, stfpf, pref, finf, chkfname,
    ...
    #> chenjf
        ...
        crds = get_crds_from_orca(logfile + '.xyz', len(atids))
    ...
    #> chenjf
        ...
        fcmatrix = get_matrix_from_orca(logfile + '.hess', 3*len(atids))
    ...
    print("==================================================================")
    print("To cite the Seminario method please use the following reference:")
    print("  J. M. Seminario, Int. J. Quantum Chem., 1996, 30, 1271-1277.")

#-----------------------------------------------------------------------------
# Modified Seminario method for the angle force constants
# J. Chem. Theory Comput. 2018, 14, 274−281
#-----------------------------------------------------------------------------

def get_mod_ang_fc(afcmatrix12, afcmatrix32, vecPA,vecPC, dis12, dis32, angle_scal1, angle_scal2):
    eigval12, eigvector12 = eig(afcmatrix12)
    eigval32, eigvector32 = eig(afcmatrix32)
    contri12 = 0.0
    contri32 = 0.0
    for i in range(0, 3):
        ...
        contri12 = contri12 + eigval12[i] * abs(dot(vecPA, ev12))
        contri32 = contri32 + eigval32[i] * abs(dot(vecPC, ev32))
    contri12 = contri12/angle_scal1
    contri32 = contri32/angle_scal2
...
def get_ang_fc_with_mod_sem(crds0, crds, fcmatrix, nat1, nat2, nat3, scalef, angavg, angle_scal1, angle_scal2):
    ...
    for i in range(0, 3):
        for j in range(0, 3):
            afcmatrix32[i][j] = -fcmatrix[3*(nat3-1)+i][3*(nat2-1)+j]
    fcfinal1 = get_mod_ang_fc(afcmatrix12, afcmatrix32, vecPA, vecPC, dis12, dis32, angle_scal1, angle_scal2)
    ...
    if angavg == 1:
        ...
        for i in range(0, 3):
            for j in range(0, 3):
                afcmatrix32[i][j] = -fcmatrix[3*(nat3-1)+i][3*(nat2-1)+j]
        fcfinal2 = get_mod_ang_fc(afcmatrix12, afcmatrix32, vecPA, vecPC, dis12, dis32, angle_scal1, angle_scal2)
        #Hatree to kcal/mol
        #Times 0.5 factor since AMBER use k(r-r0)^2 but not 1/2*k*(r-r0)^2
        ...
        for i in range(0, 3):
            for j in range(0, 3):
                afcmatrix32[i][j] = -fcmatrix[3*(nat2-1)+i][3*(nat3-1)+j]
        fcfinal3 = get_mod_ang_fc(afcmatrix12, afcmatrix32, vecPA, vecPC, dis12, dis32, angle_scal1, angle_scal2)
        ...
        for i in range(0, 3):
            for j in range(0, 3):
                afcmatrix32[i][j] = -fcmatrix[3*(nat2-1)+i][3*(nat3-1)+j]
        fcfinal4 = get_mod_ang_fc(afcmatrix12, afcmatrix32, vecPA, vecPC, dis12, dis32, angle_scal1, angle_scal2)
...
def u_PA_from_angles(nat1, nat2, nat3, crds):
    crd1 = crds[3*nat1-3:3*nat1]
    crd2 = crds[3*nat2-3:3*nat2]
    crd3 = crds[3*nat3-3:3*nat3]
    dis12 = calc_bond(crd1, crd2) #unit is bohr
...
def get_angle_scaling_list(alist, crds, atids, natids):
    #This function uses the modified Seminario method to find the angle
    #parameters and print them to file

    fcfinal = zeros(len(alist))
    angval = zeros(len(alist))
    unique_values_angles = [] #Used to find average values

    #
    #Modified Seminario part goes here ...
    #
    #Connectivity information for the Modified Seminario Method
    central_atoms_angles = []
    # The length of this dict is the same as the number of atoms in the system

    if len(crds)/3 != len(atids):
        raise pymsmtError('Error: the coordinates and atom IDs are mismatch '
                          'for the small model!')
    ...
    #ie. central_atoms_angles[3] contains an array of angles with
    #central atom 3
    for i in range(0, len(atids)):
        central_atoms_angles.append([])
        for j in range(0, len(nalist)):
            if i == nalist[j][1]: # if i is the central atom
                #For angle ABC, atoms A C are written to array
                AC_array = [nalist[j][0], nalist[j][2], j]
                central_atoms_angles[i].append(AC_array)

                #For angle ABC, atoms C A are written to array
                CA_array = [nalist[j][2], nalist[j][0], j]
                central_atoms_angles[i].append(CA_array)

    # central_atoms_angles list contains the re-sequenced atom IDs: 0, 1, 2...

    #Sort rows by atom number
    for i in range(0, len(atids)):
    #for ind_i in range(0, len(atids)):
        #i = atids[ind_i]
    ...
    for i in range(0, len(central_atoms_angles)):
        ...
        for j in range(0, len(central_atoms_angles[i])):
            ...
            unit_PA_all_angles[i].append(u_PA_from_angles(central_atoms_angles[i][j][0] + 1,
                i + 1, central_atoms_angles[i][j][1] + 1, crds))
            # The above + 1 is an offset here to match with the natids

    #Finds the contributing factors from the other angle terms
    #scaling_factor_all_angles = cell(max(max(nalist)));
    ...
    for i in range(0, len(central_atoms_angles)):
        ...
        for j in range(0, len(central_atoms_angles[i])):
            ...
            #Forwards directions, finds the same angles with the central atom i  
            while(( (j+n) < len(central_atoms_angles[i]) ) and
                central_atoms_angles[i][j][0] == central_atoms_angles[i][j+n][0] ):
                additional_contributions = additional_contributions + \
                        (abs(dot(unit_PA_all_angles[i][j][:], unit_PA_all_angles[i][j + n][:])))**2
                n = n + 1
                angles_around = angles_around + 1

            #Backwards direction, finds the same angles with the central atom i   
            while(( (j-m) >= 0 ) and
                central_atoms_angles[i][j][0] == central_atoms_angles[i][j-m][0] ):
                additional_contributions = additional_contributions + \
                        (abs(dot(unit_PA_all_angles[i][j][:], unit_PA_all_angles[i][j - m][:])))**2
                m = m + 1
                angles_around =  angles_around + 1
            ...
            if (n != 1 or m != 1):
                ...
                #The following equation corresponds to the numerator of the
                #first term in the right side of equation 12 in the
                #paper of modified Seminario method (JCTC, 2018, 14, 274-281)
                #where (m + n - 2) here equals to N - 1,
                #and additional_contributions equal to the sum(u_PA1*u_PAi) - 1.
                #Here -1 one is not showing up is because the sum does not
                #include u_PA1*u_PA1
                scaling_factor_all_angles[i][j][0] = 1 + ( additional_contributions / (m  + n - 2) )   
            else:
                scaling_factor_all_angles[i][j][0] = 1

    # Generate a blank list for scaling factors for each angle
    angle_scaling_list = []
    for i in range(0, len(nalist)):
        angle_scaling_list.append([])

    # Orders the scaling factors according to the angle list
    for i in range(0, len(central_atoms_angles)):
        for j in range(0, len(central_atoms_angles[i])):
            # scaling_factor_all_angles[i][j][1] is the position of the angle in the angle list
            # scaling_factor_all_angles[i][j][0] is the scaling factor
            angle_scaling_list[scaling_factor_all_angles[i][j][1]].append(scaling_factor_all_angles[i][j][0])

    return angle_scaling_list

def gene_by_QM_fitting_mod_sem(smpdbf, ionids, xstru, stfpf, pref, finf,
                        chkfname, logfile, g0x, scalef, bondavg, angavg):

    print("******************************************************************")
    print("*                                                                *")
    print("*====Using the Modified Seminario method to solve the problem====*")
    print("(Only the angle parameters are different from the Seminario method)")
    print("*                                                                *")
    print("******************************************************************")
    ...
    #> chenjf
        ...
        crds = get_crds_from_orca(logfile)
    ...
    #> chenjf
        ...
        fcmatrix = get_matrix_from_orca(logfile, 3*len(atids))
    ...
    #for angle
    print("=======================Generate the angle parameters==============")
    angle_scaling_list = get_angle_scaling_list(alist, crds, atids, natids)

    for misang in missangtyps:
        angvals = []
        afconst = []
        for i in range(0, len(alist)):
            angle_scal1 = angle_scaling_list[i][0]
            angle_scal2 = angle_scaling_list[i][1]
            ang = alist[i]
            at1 = ang[0]
            at2 = ang[1]
            at3 = ang[2]
            ...
            if angtyp == misang or angtyp[::-1] == misang:
                ...
                if angavg == 1:
                    angval, fcfinal, stdv = get_ang_fc_with_mod_sem(crds0, crds, fcmatrix, nat1, nat2, nat3,
                                                                    scalef, angavg, angle_scal1, angle_scal2)
                    print('### Angle force constant between ' + \
                      mol.atoms[at1].resname + str(mol.atoms[at1].resid) +  '@' + mol.atoms[at1].atname + ', ' + \
                      mol.atoms[at2].resname + str(mol.atoms[at2].resid) +  '@' + mol.atoms[at2].atname + ' and ' + \
                      mol.atoms[at3].resname + str(mol.atoms[at3].resid) +  '@' + mol.atoms[at3].atname + ' : ' + \
                    ...
                    angval, fcfinal = get_ang_fc_with_mod_sem(crds0, crds, fcmatrix, nat1, nat2, nat3,
                                                              scalef, angavg, angle_scal1, angle_scal2)
    ...
    #Print out the final frcmod file
    print_frcmod_file(pref, finf, finalparmdict, 'Modified Seminario')

    print("==================================================================")
    print("To cite the modified Seminario method please use the following reference:")
    print("  A. E. Allen, M. C. Payne and D. J. Cole. J. Chem. Theory Comput.,")
    print("  2018, 14, 274-281.")

    #############################################################
    ######### Need to modify this part in the next time #########
    #############################################################
    #
    ...
    #    [ABC_fcfinal, ABC_angval] = get_ang_fc_with_mod_sem(crds0, crds, fcmatrix,
    #        alist[i][0], alist[i][1], alist[i][2], scalef, angavg,
    #        angle_scaling_list[i][0], angle_scaling_list[i][1])
    #    [CBA_fcfinal, CBA_angval] = get_ang_fc_with_mod_sem(crds0, crds, fcmatrix,
    #        alist[i][2], alist[i][1], alist[i][0], scalef, angavg,
    #        angle_scaling_list[i][1], angle_scaling_list[i][0])
    #  
    #    fcfinal[i] = ( ABC_fcfinal + CBA_fcfinal ) / 2
    #    angval[i] = ( ABC_angval +  CBA_angval ) / 2
    #  
    #    mol, atids, resids = get_atominfo_fpdb(pdb)
    #    at1_rep = atom_rep(mol, alist[i][0])
    #    at2_rep = atom_rep(mol, alist[i][1])
    #    at3_rep = atom_rep(mol, alist[i][2])
    #  
    #    #Vibrational_scaling takes into account DFT deficities/ anharmocity 
    #    fcfinal[i] =  fcfinal[i] * scalef * scalef
    #  
    #############################################################
    ##### End here ##############################################
    #############################################################
