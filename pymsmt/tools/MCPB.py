...
** P. Li and K. M. Merz, Jr. JCIM, 2016, 56, 599-604.

The parameterization scheme is adapted from:
** M. B. Peters, Y. Yang, B. Wang, L. Fusti-Molnar, M. N. Weaver, and 
   K. M. Merz, Jr. JCTC, 2010, 6, 2935-2947.
...
The modified Seminario method is from:
** A. E. Allen, M. C. Payne and D. J. Cole. JCTC, 2018, 14, 274-281.

The empirical method is from:
** Z. Yu, P. Li, and K. M. Merz, Jr. JCTC, 2018, 14, 242-254.

The fluctuating charge model is from:
** L. Landry and P. Li, JCIM, 2024, 64, 812-824.
"""
#==============================================================================
...
from pymsmt.mcpb.fq_model import cal_fq_chg
...
from pymsmt.mcpb.gene_final_frcmod_file import (gene_by_empirical_way,
        gene_by_QM_fitting_sem, gene_by_QM_fitting_mod_sem,
        gene_by_QM_fitting_zmatrix, gene_blank)
...
if options.step not in ['1', '1n', '1m', '1a', '2', '2e', '2s', '2ms', '2z',
                        '2b', '3', '3a', '3b', '3c', '3d', '3f', '4', '4b',
                        '4n1', '4n2']:
    raise pymsmtError('Invalid step number chosen. please choose among the '
                      'following values: 1, 1n, 1m, 1a, 2, 2e, 2s, 2ms, 2z, 3, '
                      '3a, 3b, 3c, 3d, 3f, 4, 4b, 4n1, 4n2')
...
for line in inputf:
    ...
    #software_version
        ...
        if len(line) == 2:
            g0x = line[1].lower()
            if g0x not in ['g03', 'g09', 'g16', 'gau', 'gms', 'orca']:
                raise pymsmtError('Please use either g03, g09, g16, gau, gms, '
                                  'or orca, other software/versions are not '
...
if options.logfile is not None:
    ...
    if g0x == 'orca':
        #chenjf
        # do not set options.logfile !!!
        # because orca does not write info to .log file
        fclogf = gname + '_orca_small_opt'
        mklogf = gname + '_orca_large_mk'
    else:
...
#2ms) Modified Seminario method by Cole et al. JCTC, 2018, 14, 274
#2z) Z-matrix method
elif (options.step in ['2', '2s', '2ms', '2e', '2z', '2b']):
    ...
    elif (options.step in ['2ms']):
        gene_by_QM_fitting_mod_sem(smpdbf, ionids, xstru, stfpf, prefcdf,
            finfcdf, fcfchkf, fclogf, g0x, scalef, bondfc_avg, anglefc_avg)
...
elif (options.step == '3f'):
    cal_fq_chg(stpdbf, lgpdbf, stfpf, ionids, ff_choice, premol2fs,
        mcresname)
